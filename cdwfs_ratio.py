...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 18 14:17:20 2020
...
for j in range(0,len(xfull_pos)):
    ...
    if os.path.isfile("../ratios/"+str(xid)+"_ratio.txt") == True: ## change directory ##
        
        
    
        f = open("../ratios/"+str(xid)+"_ratio.txt","r")
        lines=f.readlines()
        ener_obs = []
        flux_obs = []
        ener_obs_err = []
...
cdwfs_sel = array_ratio[1:,:] 

df_xsel = pd.DataFrame.from_records(cdwfs_sel)

df_xsel.rename(columns={200: 'xid', 201:'xra',202:'xdec',203:'z', 204:'hbc'}, inplace= True)
...
all_pts = np.asarray(df_xsel[["xra","xdec"]])
...
#%% Detected vs Undetected Selection -------------------------------------------

ot_detect = df_cross.loc[sp_ot_cross_ind]
ot_all = df_xsel.loc[sp_ot_all_ind]

#ave_hb = ot_all[list(a)].mean(axis=1)
...
#ax[0].plot(ener,f_ot_low, "b--", linewidth=1)
#ax[1].plot(ener,f_ot_high, "b--", linewidth=1)
...
# xdata, ydata, fungau, funmod, popt, width = fefit(np.asarray(f_in_high))
